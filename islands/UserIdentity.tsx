...
export default function UserIdentity() {
  ...
  if (name) {
    return (
      <div
        class="fixed top-4 right-4 z-50 bg-gray-800/80 backdrop-blur px-4 py-2 rounded-full border border-gray-700 text-sm text-gray-300 flex items-center gap-2 group cursor-pointer hover:bg-gray-700 transition-colors"
        onClick={() => {
      ...
      <div class="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-sm animate-fade-in">
        <div class="bg-gray-800 p-8 rounded-2xl border border-gray-700 shadow-2xl max-w-sm w-full mx-4 animate-scale-up">
          ...
          <p class="text-gray-400 mb-6 text-sm">
            Enter your name to track your progress and join the leaderboard.
          </p>
          ...
          <form onSubmit={handleSave} class="space-y-4">
            ...
            <button
              type="submit"
              class="w-full py-3 bg-emerald-600 hover:bg-emerald-500 rounded-xl font-bold text-white shadow-lg shadow-emerald-500/20 transition-all hover:scale-[1.02]"
            >
              Start Journey
            </button>
